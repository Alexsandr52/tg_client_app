...
// tg.MainButton.onClick(check_list);
...
function getTimeByCountryCode(countryCode) {
  const now = new Date();
  now.toLocaleString("en-US", { timeZone: countryCode });
  const time = now.toLocaleTimeString();
  let message = "";
  if (parseInt(time.toString().split(":")[0]) >= 12) {
    message = "Добрый день ";
  } else if (parseInt(time.toString().split(":")[0]) >= 16) {
    message = "Добрый вечер ";
  } else if (parseInt(time.toString().split(":")[0]) < 6) {
    message = "Доброй ночи ";
  } else {
    message = "Доброе утро ";
  }
  return message;
  console.log(`Текущее время в ${countryCode}: ${time}`);
}
...
// let dates = {"27.02.2024":{"11:00":[["1","1","Свободно"],["1","2","Свободно"],["1","3","Свободно"],["2","1","Забронировано"],["2","2","Забронировано"],["2","3","Свободно"],["3","1","Забронировано"],["3","2","Свободно"],["3","3","Свободно"]],"15:00":[["1","1","Забронировано"],["1","2","Свободно"],["1","3","Свободно"],["1","4","Забронировано"],["2","1","Свободно"],["2","2","Забронировано"],["2","3","Свободно"],["2","4","Забронировано"],["3","1","Свободно"],["3","2","Свободно"],["3","3","Забронировано"],["3","4","Забронировано"],["4","1","Свободно"],["4","2","Забронировано"],["4","3","Забронировано"],["4","4","Забронировано"],["4","5","Забронировано"],["4","6","Свободно"]]},"28.02.2024":{"17:00":[["1","1","Забронировано"],["1","2","Свободно"],["1","3","Свободно"],["1","4","Свободно"],["1","5","Забронировано"],["1","6","Забронировано"],["2","1","Забронировано"],["2","2","Забронировано"],["2","3","Забронировано"]],"23:00":[["2","4","Свободно"],["2","5","Забронировано"],["2","6","Забронировано"],["3","1","Забронировано"],["3","2","Забронировано"],["3","3","Забронировано"],["3","4","Забронировано"],["3","5","Свободно"],["3","3","Забронировано"]]},"29.02.2024":{"23:50":[["1","1","Забронировано"],["1","2","Свободно"],["1","3","Забронировано"],["1","4","Забронировано"],["1","5","Свободно"],["1","6","Свободно"],["2","1","Свободно"],["2","2","Забронировано"],["2","3","Свободно"],["2","4","Свободно"],["2","5","Забронировано"],["2","6","Свободно"],["3","1","Свободно"],["3","2","Свободно"],["3","3","Забронировано"],["3","4","Забронировано"],["3","5","Свободно"],["3","6","Забронировано"]]}}
// main_view_function(dates);
...
function main_view_function(dates) {
  
  const valid_days = Object.keys(dates).map((dateString) => {
  ...
  function make_price_list() {
    ...
    price_list.querySelector('.time_header_1').innerHTML = `${tg.initDataUnsafe.user.first_name}`
...
// // Обработчики для места
// function click_handler_second(e) {
//   if (!Object.keys(temp_choice).includes(e.target.parentNode.id)) {temp_choice[e.target.parentNode.id] = [];}
//   temp_choice[e.target.parentNode.id].push(e.target.id);

//   if (temp_choice[e.target.parentNode.id].indexOf(e.target.id) !== -1) {
//     temp_choice[e.target.parentNode.id] = temp_choice[e.target.parentNode.id].filter(function (item) {return item !== e.target.id;});
//     if (temp_choice[e.target.parentNode.id].length == 0) {delete temp_choice[e.target.parentNode.id];}
//   }
...
//     switch (box_name){
//       case 'price_list': price_list.style.display = "block"; break;
//       case 'cinema_view': cinema_view.style.display = "block"; break;
//       case 'time_view': time_view.style.display = "block"; break;
//       case 'calendar_view': calendar_view.style.display = "block"; break;
//     }
//   }
...
//     for (let i = 1; i <= 30; i++) {
//       if (valid_days.includes(i.toString())) {calendar.innerHTML += `<div id="${i}" class="active">${i}</div>`;
//       }else {calendar.innerHTML += `<div id="${i}" class="disabled-celendar">${i}</div>`;
//       }
//     }
...
//     for (let el in valid_days) {
//       document.getElementById(valid_days[el]).addEventListener('click', (e) => {
//         user_input['date_choice'] = Object.keys(dates)[valid_days.indexOf(e.target.id)];
//         set_time(user_input['date_choice']);
//       });
//     }
//   }
...
//   function set_time(date) {
//     tg.BackButton.show();
//     tg.BackButton.onClick(function () {make_calendar()});
...
//     for (let i in time) {document.querySelector('.avalibal_time').innerHTML += `<div class="time_item" id="${100 + i}">${time[i]}</div>`;}
...
//     for (let i in palaces) {
//       if (Object.keys(cinema_cheme).includes(palaces[i][0])) {cinema_cheme[palaces[i][0]][palaces[i][1]] = palaces[i][2];
//       } else {
//         cinema_cheme[palaces[i][0]] = {};
//         cinema_cheme[palaces[i][0]][palaces[i][1]] = palaces[i][2];
//       }
...
//       for (let id in cinema_cheme[i]) {
//         let status = cinema_cheme[i][id];
//         if (status == "Свободно") {plases_string += `<div id="${id}" class="active active_place">${id}</div>`;
//         } else {plases_string += `<div id="${id}" class="disabled-celendar">${id}</div>`;}
//       }
...
//     let elements = document.querySelectorAll(".active_place");
//     for (let i = 0; i < elements.length; i++) {elements[i].addEventListener("click", click_handler);}
//   }
